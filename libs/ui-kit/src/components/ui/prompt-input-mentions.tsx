...
export interface MentionTriggerConfig {
  ...
  renderSuggestion?: (suggestion: any, search: string, highlightedDisplay: React.ReactNode, index: number, focused: boolean) => React.ReactNode;
}
...
export const PromptInputMentions = React.forwardRef<HTMLTextAreaElement, PromptInputMentionsProps>(
  ({
    value = '',
    onChange,
    onKeyDown,
    ...
    const handleChange = React.useCallback((event: any, newValue: string) => {
        ...
        >
          {mentionTriggers.map((config) => (
            <Mention
              ...
              data={config.data}
              markup={config.markup || `${config.trigger}[__display__](${config.trigger}__id__)`}
              displayTransform={config.displayTransform || ((id, display) => `${config.trigger}${display}`)}
