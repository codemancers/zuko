    ...
    >
      <span
        ...
        className={clsx([
          className,
          // Basic layout
          isPlain ? 'relative block' : 'relative block w-full',
          // Background color + shadow applied to inset pseudo element, so shadow blends with border in light mode
          !isPlain &&
            'before:absolute before:inset-px before:rounded-[calc(var(--radius-lg)-1px)] before:bg-white before:shadow-sm',
          // Background color is moved to control and shadow is removed in dark mode so hide `before` pseudo
          !isPlain && 'dark:before:hidden',
          // Focus ring
          !isPlain &&
          ...
          !isPlain &&
            'has-data-disabled:before:bg-zinc-950/5 has-data-disabled:before:shadow-none',
          // Invalid state
          !isPlain && 'has-data-invalid:before:shadow-red-500/10',
        ])}
      >
        <Headless.ComboboxInput
          ...
          className={clsx([
            // Basic layout
            isPlain
              ? 'relative block w-full bg-transparent p-0 text-sm/6 text-zinc-950 dark:text-white placeholder:text-zinc-500 focus:outline-hidden border-none'
              : clsx(
                  className,
        ...
        >
          <svg
            className={clsx(
              'stroke-zinc-500 group-data-disabled:stroke-zinc-600 group-data-hover:stroke-zinc-700 dark:stroke-zinc-400 dark:group-data-hover:stroke-zinc-300 forced-colors:stroke-[CanvasText]',
              isPlain ? 'size-3.5' : 'size-5 sm:size-4',
            )}
            viewBox="0 0 16 16"
            aria-hidden="true"
            fill="none"
