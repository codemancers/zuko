...
export class DealsService {
  ...
  async create(input: CreateDealInput) {
    ...
    this.logger.log('[SERVICE] All validations passed, creating deal in database');
    try {
      const result = await this.dealsRepository.create(input);
      this.logger.log(`[SERVICE] Deal created successfully with ID: ${result.id}`);
      return result;
    } catch (error: unknown) {
  ...
  async update(id: number, input: UpdateDealInput) {
    ...
    if (input.actualCloseDate && input.expectedCloseDate) {
      if (input.actualCloseDate < input.expectedCloseDate) {
        this.logger.warn('[SERVICE] Actual close date is before expected close date');
      }
    }
  ...
  async addAccount(dealId: number, input: AddAccountToDealInput) {
    this.logger.log(`[SERVICE] Adding account ${input.accountId} to deal ${dealId}`);

    await this.findById(dealId);

    // Check if account is already associated with this deal
    const existingAccounts = await this.dealsRepository.getAccounts(dealId);
    const alreadyAssociated = existingAccounts.find((da) => da.accountId === input.accountId);

    if (alreadyAssociated) {
      throw new BadRequestException(
        `Account ${input.accountId} is already associated with this deal`
      );
    }

    try {
      const result = await this.dealsRepository.addAccount(dealId, input);
      this.logger.log(
        `[SERVICE] Account ${input.accountId} added to deal ${dealId} successfully`
      );
      return result;
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(
        `[SERVICE] Failed to add account to deal: ${errorMessage}`,
        errorStack
      );
      throw error;
    }
  ...
  async removeAccount(dealId: number, accountId: number) {
    this.logger.log(`[SERVICE] Removing account ${accountId} from deal ${dealId}`);

    await this.findById(dealId);

    try {
      const result = await this.dealsRepository.removeAccount(dealId, accountId);
      this.logger.log(
        `[SERVICE] Account ${accountId} removed from deal ${dealId} successfully`
      );
      return result;
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      const errorStack = error instanceof Error ? error.stack : undefined;
      this.logger.error(
        `[SERVICE] Failed to remove account from deal: ${errorMessage}`,
        errorStack
      );
      throw error;
    }
  ...
  async updateAccount(dealId: number, accountId: number, isPrimary: boolean) {
    await this.findById(dealId);
    return this.dealsRepository.updateAccount(dealId, accountId, isPrimary);
  }

  async getAccounts(dealId: number) {
    await this.findById(dealId);
    return this.dealsRepository.getAccounts(dealId);
  }

  async addContact(dealId: number, input: AddContactToDealInput) {
    this.logger.log(`[SERVICE] Adding contact ${input.contactId} to deal ${dealId}`);
    ...
    const alreadyAssociated = existingContacts.find((dc) => dc.contactId === input.contactId);
  ...
  async removeContact(dealId: number, contactId: number) {
    this.logger.log(`[SERVICE] Removing contact ${contactId} from deal ${dealId}`);

    await this.findById(dealId);

    try {
      const result = await this.dealsRepository.removeContact(dealId, contactId);
      this.logger.log(
  ...
  async getDealsByAccount(accountId: number, pagination?: PaginationOptions) {
    return this.dealsRepository.getDealsByAccount(accountId, pagination);
  }
