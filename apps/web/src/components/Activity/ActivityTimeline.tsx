...
export default function ActivityTimeline({
  ...
  const updateActivityMutation = useMutation({
    mutationFn: ({ activityId, content }: { activityId: number; content: string }) => {
      ...
      return activitiesApi.updateActivity(activityId, { content, userId: currentUserId });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["timeline", entityType, entityId],
  ...
  const handleSave = (activityId: number) => {
    if (editedContent.trim() && editedContent.trim() !== activities?.find(a => a.id === activityId)?.content) {
      updateActivityMutation.mutate({ activityId, content: editedContent.trim() });
    }
  };
      ...
      <div className="space-y-0">
        {!activities || activities.length === 0 ? (
          ...
          activities.map((activity, index) => (
            <div key={activity.id} className="flex gap-3 pb-6" data-testid="activity-item">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex-1">
                    ...
                    {activity.activityType === "comment" && activity.content && (
                      <>
                          ...
                          <div className="mt-2 space-y-2">
                            ...
                            <div className="flex gap-2">
                              <Button
                                ...
                                disabled={!editedContent.trim() || updateActivityMutation.isPending}
                              >
                  ...
                  {activity.activityType === "comment" &&
                    ...
                    editingActivityId !== activity.id && (
                      <button
                        onClick={() => handleEdit(activity.id, activity.content || "")}
                        disabled={updateActivityMutation.isPending}
