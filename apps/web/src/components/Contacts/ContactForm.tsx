...
import { contactsApi, type Contact, type CreateContactDto, type UpdateContactDto } from "@/lib/api/contacts";
...
export default function ContactForm({ contact, mode, currentUserId }: ContactFormProps) {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: UpdateContactDto) => contactsApi.updateContact(contact!.id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["contacts"] });
  ...
  const validateForm = () => {
    ...
    if (!formData.email && !formData.phone && !formData.linkedinId) {
      newErrors.contactMethod = "At least one contact method (email, phone, or LinkedIn) is required";
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {errors.contactMethod && (
        <div className="rounded-md bg-red-50 p-3 dark:bg-red-900/20">
          <p className="text-sm text-red-800 dark:text-red-200">{errors.contactMethod}</p>
        </div>
      )}
