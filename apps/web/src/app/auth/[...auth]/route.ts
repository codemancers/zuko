...
async function proxyToBackend(
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      ...
      if (location) {
        ...
        cookies.forEach((cookie) => {
          ...
          console.log('[AUTH PROXY] Rewriting cookie on redirect:', { original: cookie, rewritten: rewrittenCookie });
    ...
    cookies.forEach((cookie) => {
      ...
      console.log('[AUTH PROXY] Rewriting cookie:', { original: cookie, rewritten: rewrittenCookie });
