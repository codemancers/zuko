...
test.describe('Meetings - Authenticated', () => {
  ...
  test('add meeting form shows validation for empty name', async ({ page }) => {
    await page.goto('/meeting/add');
    await page
      .getByPlaceholder(/paste meeting url/i)
  ...
  test('meeting detail shows tabs', async ({ meetingDetailPage, page }) => {
    ...
    await expect(
      page.getByRole('tab', { name: /action items/i }),
    ).toBeVisible();
  });
  ...
  test('meeting detail Action Items tab shows list and search', async ({
    ...
    await expect(page.getByPlaceholder(/search action items/i)).toBeVisible();
  });
  ...
  test('can open delete dialog and cancel', async ({ meetingsPage, page }) => {
    await meetingsPage.goto();
