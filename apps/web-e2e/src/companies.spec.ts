...
test.describe('Companies - Authenticated', () => {
  ...
  test('creates a new company record in table', async ({
    companiesPage,
    page,
  }) => {
    await companiesPage.goto();
...
test.describe.serial('Company Detail - Contact Management', () => {
  ...
  test('filters out already associated contacts from dropdown', async ({
    ...
    const matchingOptions = selectableOptions.filter({
      hasText: firstContactName,
    });
    ...
    if ((await selectableOptions.count()) === 0) {
      await expect(
        dialog.getByText(
          /All contacts are already associated with this company/i,
        ),
      ).toBeVisible();
      await expect(
        dialog.getByRole('button', { name: /Add Contact/i }),
      ).toBeDisabled();
    } else {
  ...
  test('can remove a contact from a company', async ({ companyDetailPage }) => {
    ...
    if (contacts.length === 0) {
      ...
      const isStillVisible =
        await companyDetailPage.isContactAssociated(nameToRemove);
...
test.describe('Company Detail - Inline Editing with Activity Verification', () => {
  ...
  test('can set/edit company website', async ({ companyDetailPage, page }) => {
    ...
    await expect(
      companyDetailPage.propertyRow('Website').locator('dd'),
    ).toHaveText(newWebsite);
    ...
    await companyDetailPage.expectActivityEntry(
      `set website to "${newWebsite}"`,
    );

    // Test updating the value
    const updatedWebsite = `https://acme-${Date.now()}-updated.com`;
    await companyDetailPage.updateProperty(
      'Website',
      updatedWebsite,
      companyId,
    );
    await expect(
      companyDetailPage.propertyRow('Website').locator('dd'),
    ).toHaveText(updatedWebsite);
    await companyDetailPage.expectActivityEntry(
      `updated website from "${newWebsite}" to "${updatedWebsite}"`,
    );
    ...
    await expect(companyDetailPage.getPropertyLink('Website')).toHaveAttribute(
      'href',
      updatedWebsite,
    );
  });
  ...
  test('can set/edit company linkedin', async ({ companyDetailPage, page }) => {
    ...
    await expect(
      companyDetailPage.propertyRow('LinkedIn').locator('dd'),
    ...
    await expect(link).toHaveAttribute('target', '_blank');
    await companyDetailPage.expectActivityEntry(
      `set linkedinUrl to "${newLinkedin}"`,
    );
    ...
    await companyDetailPage.expectActivityEntry(
      `updated linkedinUrl from "${newLinkedin}" to "${updatedLinkedin}"`,
    );
    ...
    await expect(companyDetailPage.getPropertyLink('LinkedIn')).toHaveAttribute(
      'href',
      updatedLinkedin,
    );
  });
  ...
  test('shows validation error for invalid linkedin URL', async ({
    ...
    await expect(page.getByText(/Must be a valid LinkedIn URL/i)).toBeVisible({
      timeout: 3000,
    });
  });
...
test.describe('Row Creation Flow', () => {
  test('Creates new company using the sheet flow', async ({
    ...
    const companyNameIndex = headerTexts.findIndex((h) =>
    ...
    await expect(newCompanyRow.locator('td').nth(ownerIndex)).toContainText(
      'E2E Test User',
    );

    // Validate Created Date field (DD MMM YYYY)
    const formattedDate = dayjs().format('DD MMM YYYY');
    await expect(newCompanyRow.locator('td').nth(createdIndex)).toContainText(
      formattedDate,
    );
  });
});
...
test.describe('Cell Editing Flow', () => {
  test('Edits company name cell value (entity type)', async ({
    companiesPage,
    page,
  }) => {
    await companiesPage.goto();
  ...
  test('Edits company website cell value (text type)', async ({
    companiesPage,
    page,
  }) => {
    await companiesPage.goto();
    ...
    const headers = page.getByRole('columnheader');
    const headerTexts = await headers.allInnerTexts();
    const websiteIndex = headerTexts.findIndex((h) =>
  ...
  test('Quits editing mode when Escape key is pressed', async ({
    companiesPage,
    page,
  }) => {
    await companiesPage.goto();
    ...
    const headers = page.getByRole('columnheader');
    const headerTexts = await headers.allInnerTexts();
    const linkedinIndex = headerTexts.findIndex((h) =>
