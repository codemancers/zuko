...
pnpm exec nx run @zuko/backend:build         # Production build
pnpm exec nx run @zuko/backend:test          # Unit tests
pnpm exec nx run @zuko/backend:lint          # Lint

# Database
pnpm exec nx run @zuko/models:prisma:generate                   # Regenerate Prisma client
pnpm exec nx run @zuko/models:prisma:migrate -- --name <name>   # New migration
pnpm exec nx run @zuko/models:prisma:studio                     # DB browser (port 5555)
