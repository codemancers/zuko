...
<Note>
  ...
  the Next.js process, use `pnpm exec nx run @zuko/web:next:dev`.
</Note>
...
pnpm exec nx run @zuko/web:build      # Production build
pnpm exec nx run @zuko/web:test       # Unit tests
pnpm exec nx run @zuko/web:lint       # Lint

# E2E tests
NODE_ENV=test pnpm exec nx run web-e2e:e2e       # Headless
NODE_ENV=test pnpm exec nx run web-e2e:e2e --ui  # With UI
```
