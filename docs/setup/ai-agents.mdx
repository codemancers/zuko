...
<Note>
  The first start may be slow — the LangGraph CLI downloads additional
...
<Accordion title="Port already in use">
  ```bash lsof -ti:8080 | xargs kill -9 ```
</Accordion>

<Accordion title="Enable LangSmith tracing">
  To enable tracing for debugging: ```env LANGSMITH_TRACING=true
  LANGSMITH_API_KEY=your-langsmith-api-key LANGSMITH_PROJECT=zuko ``` Sign up at
  [smith.langchain.com](https://smith.langchain.com) to get an API key.
</Accordion>
