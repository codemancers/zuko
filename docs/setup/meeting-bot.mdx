...
```bash
pnpm exec nx run @zuko/meeting-bot:dev
```
...
```bash
pnpm exec nx run @zuko/meeting-bot:dev      # Start with hot reload
pnpm exec nx run @zuko/meeting-bot:build    # Production build
pnpm exec nx run @zuko/meeting-bot:lint     # Lint
```
