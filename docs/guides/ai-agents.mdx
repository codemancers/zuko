...
- `OPENAI_API_KEY` and `OPENAI_MODEL` — model credentials and config for the agent runtime

The backend must also have the same `AGENT_TOKEN` configured so it can validate incoming requests.
...
`AgentGuard` validates the token and sets `request.organizationId` from `x-org-id`, so routes can keep using the `@OrgId()` decorator.
