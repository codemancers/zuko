...
```bash
pnpm exec nx run @zuko/models:prisma:studio
...
# Database
pnpm exec nx run @zuko/models:prisma:generate                  # Generate Prisma client
pnpm exec nx run @zuko/models:prisma:migrate -- --name init    # Run migrations
pnpm exec nx run @zuko/models:seed                             # Seed test data
pnpm exec nx run @zuko/models:prisma:studio                    # Open DB browser (port 5555)
pnpm exec nx run @zuko/models:prisma:reset                     # Reset DB (destructive!)

# Development
pnpm exec nx run @zuko/web:dev                                 # Start frontend + backend
pnpm exec nx run @zuko/backend:serve                           # Start backend only
pnpm exec nx run @zuko/ai-agents:dev                           # Start AI agents (port 8080)
pnpm exec nx run @zuko/meeting-bot:dev                         # Start meeting bot (port 8000)

# Tests
pnpm exec nx run @zuko/backend:test                            # Backend unit tests
pnpm exec nx run @zuko/web:test                                # Web unit tests
pnpm exec nx affected -t test                                  # Test only affected projects

# E2E tests
NODE_ENV=test pnpm exec nx run web-e2e:e2e                    # Run E2E tests
NODE_ENV=test pnpm exec nx run web-e2e:e2e --ui               # E2E with UI mode
...
# Utilities
pnpm exec nx graph                                             # Visualize project dependency graph
pnpm exec nx show project @zuko/backend                        # Show available targets
pnpm exec nx affected -t lint                                  # Lint affected projects
```
...
<Accordion title="pnpm install fails or lock file issues">
