...
| Monorepo    | Nx + pnpm 11 workspaces                              |
...
pnpm is the package manager across the monorepo. The version is pinned by the
`packageManager` field in the root `package.json`, so Corepack picks it up:

```bash
corepack enable pnpm
pnpm --version
# Should output 11.26.0
```
